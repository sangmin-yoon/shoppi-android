...
class HomeBannerAdapter:ListAdapter<Banner, HomeBannerAdapter.HomeBannerViewHolder>(BannerDiffCallback()) {
    ...
    override fun onBindViewHolder(holder: HomeBannerAdapter.HomeBannerViewHolder, position: Int) { // onCreateViewHolder가 호출된 이후(잘 생성이 되면) viewHolder가 인자로 전달된다.
        holder.bind(getItem(position)) // 전달된 holder에 데이터를 binding 한다.
    }

    class HomeBannerViewHolder(view: View) : RecyclerView.ViewHolder(view) { // 홈배너에서 인플레이트 시킬 view이다
